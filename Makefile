...
GNU_OPT = -Wall -Wextra -Wpedantic -Wundef -Wunused -Wshadow -Winline -Wpacked -Wcast-qual -Wcast-align -Wc++-compat -Wfloat-equal -Wswitch-enum -Wswitch-default -Wdouble-promotion
...
# source
######################################
-include Drivers/make.mk
ifeq ($(ENABLE_DSP), 1)
-include Drivers/dsp.mk
endif # ENABLE_DSP
...
OBJECTS += $(addprefix $(BUILD_DIR)/,$(notdir $(ASM2_SOURCES:.S=.o)))
vpath %.S $(sort $(dir $(ASM2_SOURCES)))
...
format: Core
	@find $^ -regex '.*\.\(c\|h\|cxx\|hxx\|cpp\|hpp\)' -exec clang-format -style=file -i {} --verbose \;
# *** EOF ***
